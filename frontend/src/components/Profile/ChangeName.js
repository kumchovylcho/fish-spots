...
import { validateUsername } from '../../util/registrationValidator';
import Spinner from '../Spinner/Spinner';
import { changeUserData, createNewToken, userExistsData } from '../../services/users';
...
export default function ChangeName({ username, setAuthTokens, setUser }) {
    ...
    const onFormSubmitHandler = async (e) => {
        ...
        setIsLoading(true);
        const checkUsernameResponse = await userExistsData({username: newName, old_username: username, password: password, check_password: true});
        ...
        if (response.ok) {
            ...
            await createNewToken({ username: data.username, password: password }, setAuthTokens, setUser);
    ...
    const OnChangeUsernameHandler = (e) => {
        ...
        if (isValid || !currentName.length) {
            ...
            setError(`${currentName[currentName.length - 1]} не е позволен символ.`);
        }
    }
    ...
    return (
        <div className="max-md:text-center flex justify-center grow text-left max-w-2xl break-words text-white p-2 rounded bg-gradient-to-b from-sky-400 to-blue-500">
            <form className="w-3/4 flex flex-col items-center gap-5" onSubmit={onFormSubmitHandler}>
                ...
                <section className="relative flex flex-col w-full">
                    ...
                    <input 
                        ...
                        <i
                            ...
                            className={`text-black fa-solid ${passwordEye ? "fa-eye" : "fa-eye-slash"} absolute right-4 bottom-[0.5rem] hover:cursor-pointer`}>
                        </i>
                </section>
                {error.length > 0 && 
